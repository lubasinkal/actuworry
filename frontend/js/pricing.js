document.addEventListener("DOMContentLoaded", () => {
	...
	const pricingSubmitButton = pricingForm.querySelector(
		'button[type="submit"]',
	);
	...
	pricingForm.addEventListener("submit", async (e) => {
		...
		try {
			...
			if (result.expenses) {
				...
				initialExpenseSpan.textContent = (
					result.expenses.initial_expense_rate * 100
				).toFixed(1);
				renewalExpenseSpan.textContent = (
					result.expenses.renewal_expense_rate * 100
				).toFixed(1);
				maintenanceExpenseSpan.textContent =
					result.expenses.maintenance_expense.toFixed(0);
				profitMarginSpan.textContent = (
					result.expenses.profit_margin * 100
				).toFixed(1);
			} else {
...
function createReserveChart(reserveSchedule, productType) {
	...
	const chartTitle =
		productType === "whole_life"
			? "Whole Life Reserve Schedule"
			: "Term Life Reserve Schedule";
	const chartColor =
		productType === "whole_life"
			? "rgba(99, 102, 241, 0.8)"
			: "rgba(59, 130, 246, 0.8)";
...
function displayPremiumResults(result) {
	...
	if (result.product_type.includes("annuity")) {
		...
		netPremiumValue.textContent = result.total_premium_cost
			? result.total_premium_cost.toFixed(2)
...
function displayUnderwritingInfo(result) {
	...
	if (!underwritingSection) {
		...
		expenseSection.parentNode.insertBefore(
			underwritingSection,
			expenseSection.nextSibling,
		);
	}
	...
	if (result.underwriting && Object.keys(result.underwriting).length > 0) {
		underwritingHTML +=
			'<h4 class="font-semibold mb-2 text-purple-800">Underwriting Factors:</h4>';
		...
		if (result.underwriting.health_rating) {
			const rating =
				result.underwriting.health_rating.charAt(0).toUpperCase() +
				result.underwriting.health_rating.slice(1);
	...
	if (result.risk_assessment) {
		underwritingHTML +=
			'<h4 class="font-semibold mb-2 mt-3 text-purple-800">Risk Assessment:</h4>';
		underwritingHTML += '<div class="grid grid-cols-2 gap-2 text-sm">';
		...
		if (result.risk_assessment.annual_death_probability) {
			const probability = (
				result.risk_assessment.annual_death_probability * 100
			).toFixed(3);
		...
		if (result.risk_assessment.expected_lifetime_years) {
			const lifetime = Math.round(
				result.risk_assessment.expected_lifetime_years,
			);
...
function displayAnnuityInfo(result) {
	...
	let annuityHTML =
		'<h4 class="font-semibold mb-3 text-green-800">Annuity Details:</h4>';
	...
	if (result.total_premium_cost) {
		const payoutYears = Math.round(
			result.total_premium_cost / result.annual_payout,
		);
		annuityHTML += `
            <div class="bg-white p-3 rounded shadow-sm">
